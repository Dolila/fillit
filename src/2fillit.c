...
void	display(s_fillit var)
{
	while (++var.n < var.nb_tetri)
	{
		if (!(var.map = place_tetri(*var.pos, var.tetrxy[var.n], var.map, var.mapx)))
		{
			*var.pos = 0;
			if (next_lexicorder(var.order, var.nb_tetri))
				var.tetrxy = sort_tetrxy(var.tetrxy_tmp, var.order, var.nb_tetri);
			else
			{
				++var.mapx;
				var.order = var.order_tmp;
				var.tetrxy = var.tetrxy_tmp;
			}
			if (var.mapx < 4)
				var.mapx++;
			free(var.map);
			var.map = build_map(var.mapx);
			var.n = -1;
		}
	}
	ft_putendl(var.map);
...
void	before_display(s_fillit var)
{
	var.mapx = h_sqrt(var.nb_tetri * 4);
	var.map = build_map(var.mapx);
	var.n = -1;
	var.order = set_order(var.nb_tetri);
	var.order_tmp = set_order(var.nb_tetri);
	var.tetrxy_tmp = var.tetrxy;
	var.pos = (int *)malloc(sizeof(var.pos));
	*var.pos = 0;
	display(var);
}
int		main(int ac, char **av)
{
	...
	if (!(var.tetrxy = tetrxy_malloc()))
		return (0);
	var.nb_tetri = 0;
	var.l = 'A';
	if ((var.nb_tetri = check_tetri_file(fd, var.nb_tetri, var.tetrxy, var.l)) == 0)
		return (0);
	before_display(var);
/*	var.mapx = h_sqrt(var.nb_tetri * 4);
	var.map = build_map(var.mapx);
